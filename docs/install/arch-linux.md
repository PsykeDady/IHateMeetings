...
The script uses official repositories for FFmpeg, Git and `uv`. It installs CPython 3.13 through `uv` and does not alter Arch's system Python. The explicit ASR model command downloads about 490 MB; the Italian alignment model requires about 1.3 GB. Use `tiny` (about 80 MB) with `--model tiny` for a smaller smoke test. Models are never downloaded by the installer.
