...
def format_timestamp(seconds: float, *, always_hours: bool = True, milliseconds: bool = False) -> str:
    if seconds < 0:
    ...
    if always_hours:
        ...
        base = f"{minutes:02d}:{secs:02d}" if hours == 0 else f"{hours:02d}:{minutes:02d}:{secs:02d}"
